...
export default function LoginPage() {
  const { loginWithGoogle, loginWithApple, signInWithEmail, loginWithEmail } = useAuth();
  const [email, setEmail] = useState("");
  const [password, setPassword] = useState("");
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const handleQuickSelect = async (qName: string, qEmail: string) => {
    setIsLoading(true);
    setError(null);
    try {
      await loginWithEmail(qEmail, qName);
    } catch (err: any) {
      setError(err?.message || "Quick select login failed.");
      setIsLoading(false);
    }
  };
  ...
  const handleSocialLogin = async (provider: "google" | "apple") => {
    ...
    try {
      ...
      let msg = err?.message || `${provider === "google" ? "Google" : "Apple"} Sign-In failed.`;
      if (err?.code === "auth/popup-closed-by-user") {
        msg = "The authentication popup was closed before completion.";
      } else if (err?.code === "auth/popup-blocked") {
        msg = "The login popup was blocked by the browser. Please allow popups for this site.";
      }
      setError(msg);
      setIsLoading(false);
    }
      ...
      <div className="w-full max-w-md bg-slate-950/40 border border-slate-900 rounded-3xl p-8 backdrop-blur-xl shadow-2xl relative z-10 space-y-6">
        <div className="text-center space-y-2">
          ...
          <p className="text-xs text-slate-400">
            {isFirebaseConfigured ? "Sign in to your account" : "Sign in to your local workspace demo account"}
          </p>
        </div>
        ...
        {!isFirebaseConfigured && (
          <div className="flex gap-2.5 items-start bg-amber-500/5 border border-amber-500/10 rounded-2xl p-3.5 text-[11px] text-amber-200/80 leading-normal">
            ...
            <span>
              <strong>Developer Mode Fallback:</strong> Firebase is unconfigured. Use a social provider, quick profile, or enter credentials to log in locally.
            </span>
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {isFirebaseConfigured && (
            <div className="space-y-1.5">
              <label className="text-[10px] uppercase font-bold tracking-wider text-slate-400 block ml-1">Password</label>
              <div className="relative">
          ...
          <button
            type="submit"
            disabled={isLoading || !email || (isFirebaseConfigured && !password)}
            ...
            <div className="grid grid-cols-1 gap-2">
              {[
                ...
                <button
                  ...
                  disabled={isLoading}
                  className="w-full text-left p-3 rounded-xl border border-slate-900 bg-slate-950/30 hover:border-indigo-500/20 hover:bg-slate-950/80 transition-smooth group flex items-center justify-between disabled:opacity-50 disabled:pointer-events-none"
                >
                  <div>
