...
import { Sparkles, ArrowRight, Mail, Lock, User, ShieldAlert, AlertCircle, GraduationCap, Briefcase, RefreshCw } from "lucide-react";
import Link from "next/link";

export default function SignupPage() {
  const { loginWithGoogle, loginWithApple, signUpWithEmail, loginWithEmail } = useAuth();
  ...
  const handleSocialLogin = async (provider: "google" | "apple") => {
    ...
    try {
      ...
      let msg = err?.message || `${provider === "google" ? "Google" : "Apple"} Sign-Up failed.`;
      if (err?.code === "auth/popup-closed-by-user") {
        msg = "The authentication popup was closed before completion.";
      } else if (err?.code === "auth/popup-blocked") {
        msg = "The sign-up popup was blocked by the browser. Please allow popups for this site.";
      }
      setError(msg);
      setIsLoading(false);
    }
      ...
      <div className="w-full max-w-md bg-slate-950/40 border border-slate-900 rounded-3xl p-8 backdrop-blur-xl shadow-2xl relative z-10 space-y-6">
        <div className="text-center space-y-2">
          ...
          <p className="text-xs text-slate-400">
            {isFirebaseConfigured ? "Sign up for a new account" : "Join the smart resume builder local demo space"}
          </p>
        </div>
        ...
        {!isFirebaseConfigured && (
          <div className="flex gap-2.5 items-start bg-indigo-500/5 border border-indigo-500/10 rounded-2xl p-3.5 text-[11px] text-indigo-200/80 leading-normal">
            ...
            <span>
              <strong>Developer Mode Fallback:</strong> Firebase is unconfigured. Use a social provider, or fill in details below to register a mock profile.
            </span>
          </div>
        )}
        ...
        <div className="relative flex py-1 items-center">
          ...
          <span className="flex-shrink mx-3 text-[9px] uppercase font-bold tracking-widest text-slate-500 font-mono">
            Or register options
          </span>
          <div className="flex-grow border-t border-slate-900/60" />
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button
            type="submit"
            disabled={isLoading || !name || !email || (isFirebaseConfigured && !password)}
